...
describe("UI 一致性 · 页脚 build 标记", () => {
  ...
  it("#main > .foot 文本含 b20260830h（v3.1.1）", () => {
    __test.render();
    const foot = win.document.querySelector("#main > .foot");
    expect(foot).toBeTruthy();
    expect(foot.textContent).toMatch(/v3\.1\.1 · b20260830h/);
  });

  it("openDrawer() 后 #drawer > .foot 文本含 b20260830h（v3.1.1）", () => {
    __test.openDrawer();
    const foot = win.document.querySelector("#drawer > .foot");
    expect(foot).toBeTruthy();
    expect(foot.textContent).toMatch(/b20260830h/);
  });
});
