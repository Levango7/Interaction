...
describe("UI 一致性 · 页脚 build 标记", () => {
  ...
  it("#main > .foot 文本含 b20260816b（v1.14.0）", () => {
    __test.render();
    const foot = win.document.querySelector("#main > .foot");
    expect(foot).toBeTruthy();
    expect(foot.textContent).toMatch(/v1\.14\.0 · b20260816b/);
  });

  it("openDrawer() 后 #drawer > .foot 文本含 b20260816b（v1.14.0）", () => {
    __test.openDrawer();
    const foot = win.document.querySelector("#drawer > .foot");
    expect(foot).toBeTruthy();
    expect(foot.textContent).toMatch(/b20260816b/);
  });
});
