// T3.3 数据统计测试
// 覆盖：calcTrend（7/30 天趋势 + 空边界）、calcSceneDist（ORDER 场景分布 + 空边界）、
//       calcChainSuccess（链成功率 + 无触发）、calcStats（汇总指标 + 空边界）、
//       renderTrendChart/renderPieChart/renderStats（SVG 渲染 + 仪表盘入口）。
...
describe("calcSceneDist() 场景分布", () => {
  it("S1: 场景分布——返回 ORDER.length 项，count/pct 正确，pct 之和约等于 100", () => {
    const win = loadApp();
    ...
    expect(dist).toHaveLength(ORDER.length);
    // 顺序与 ORDER 一致
  ...
  it("S2: 空数据边界——无任务时所有 count=0, pct=0", () => {
    const win = loadApp();
    const { calcSceneDist, ORDER } = win.__test;
    win.localStorage.clear();
    const dist = calcSceneDist();
    expect(dist).toHaveLength(ORDER.length);
    dist.forEach(d => {
...
describe("renderTrendChart / renderPieChart / renderStats 渲染", () => {
  ...
  it("R4: renderStats 渲染主内容区——含指标卡片 + 趋势图 + 饼图 + 联动触发率", () => {
    const win = loadApp();
    ...
    // 统计页已改为可自定义仪表盘（DASHBOARD_WIDGETS）：原「习惯链成功率」卡现名「联动触发率」
    expect(html).toContain("联动触发率");
    // 周/月切换按钮
  ...
  it("R5: 统计页可达——侧栏无独立入口，经系统概况卡「已完成」进入", () => {
    const win = loadApp();
    const { render, getActive, setActive, setTasks } = win.__test;
    win.localStorage.clear();
    setTasks([{ id:"t1", sc:"office", title:"测试任务", status:"done", due:"", priority:"", tags:[], doneAt:Date.now(), created:Date.now() }]);
    setActive("overview");
    render();
    // 侧栏（_buildSideMenu）只产出「总览组 + ORDER 场景叶子节点 + 应用组 + 系统组」，
    // 不存在统计入口：旧的顶级 data-sc="stats" 按钮已删除；SC_SUBMENU 中的
    // data-menu="dat-stats" 是历史遗留死配置（该表全文件零引用，见其定义处注释）。
    const side = win.document.getElementById("side");
    expect(side.querySelector('[data-sc="stats"]')).toBeNull();
    expect(side.querySelector('[data-menu="dat-stats"]')).toBeNull();
    // 真实入口：主页系统概况卡「已完成」项（data-act="stats"），处理器为 setActive("stats"); render();
    const entry = win.document.querySelector('.sys-ov-item[data-act="stats"]');
    expect(entry, "系统概况卡应有 data-act=stats 入口").toBeTruthy();
    entry.click();
    expect(getActive()).toBe("stats");
    // 主内容区应渲染统计页（仪表盘容器 + 关键指标组件）
    const main = win.document.getElementById("main");
    expect(main.innerHTML).toContain("dashHost");
