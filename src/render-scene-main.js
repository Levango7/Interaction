...
function renderMainHTML(){
  ...
  const taskForm = `<form class="form-row form-row--board" id="taskForm">
    ...
    <div class="fld fld-md"><label>${t("field.dueDate","截止日期")}</label><div class="date-pick"><input name="due" type="text" data-date-picker="1" placeholder="${t("placeholder.dueDate","选日期")}" aria-label="${t("field.dueDate","截止日期")}" autocomplete="off"><svg class="date-pick-caret" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 10l5 5 5-5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></div></div>
  ...
  const recFields = rec.fields.map(f=>{
    ...
    if(f.type==="textarea"){
      ...
      /* v3.7.33：日期字段加 ▼ 指示符 —— 用户："日期是下拉框，点击有日期卡片"。
         ...
         v3.7.43：**移除 inputmode="none"** —— 见下方 _dpOpen 处的详细说明。 */
      inp = `<div class="date-pick"><input name="${f.k}" type="text" data-date-picker="1"`
        + (f.timePicker ? ' data-time-picker="1"' : '')
        + ` placeholder="${t("placeholder.dueDate","选日期")}" aria-label="${esc(f.label)}" autocomplete="off">`
...
function _featureCardHtml(cfg){
  ...
  const form = '<div class="tool-form-grid">' + cfg.fields.map(function(f){
    ...
    /* v3.7.43：工具卡支持 type:"time" —— 与 recFields（render-scene-main.js 上方）**同一口径**：
       00:00~23:55 每 5 分钟一档共 288 项、data-editable="1"（既能选也能敲）。
       之前工具卡只把 time 落到 else → 渲染成普通文本框，于是「开始时间」被拆成独立字段后
    ...
    if(f.type === "time"){
      ...
      return '<div class="tool-field' + _fieldSpanCls(f) + '"><label>' + esc(f.label) + '</label>'
        + '<select data-f-field="' + esc(f.k) + '" data-editable="1" aria-label="' + esc(f.label) + '">' + _tOpts + '</select></div>';
    }
    /* v3.7.43：日期字段移除 inputmode="none"（详见 _dpOpen 处说明），并加 autocomplete="off"
       避免浏览器把历史输入当成表单自动填充弹层盖住日期面板。 */
    const inputType = f.type === "number" ? ' type="number" step="any"' : f.type === "date" ? ' type="text" data-date-picker="1" autocomplete="off"' + (f.timePicker ? ' data-time-picker="1"' : '') : ' type="text"';
 ...
 * 用法：<input type="text" data-date-picker="1">。（v3.7.43 起支持手输，详见下方 input 委托）
...
var _dpRaisedInput = null;            // v3.7.43：被临时抬到 overlay 之上的触发输入框（关闭时需复位）
var _dpOpenByKeyboard = false;        // v3.7.43：本次打开是否由键盘触发 → 决定要不要把焦点移进日格
/* v3.7.43：程序化移焦豁免标志（注释 B）。
   为什么需要它 —— blur 委托里有"失焦即关面板"的逻辑（本意：用户离开日期框就收起日历）。
   但 **内部程序化移焦**（_dpOpen 把焦点移入日格）也会让输入框 blur，于是"刚开就被自己关掉"。
   实测调用栈：Tab → focus 委托 → _dpOpen 建面板 → el.focus() 到日格
     → 输入框 blur → blur 委托 → _dpClose()  ← 面板消失
   曾试图用 document.activeElement / relatedTarget 判断"焦点是否落在面板内"，
   两者在 blur 派发时的取值时机不稳定，实测均未拦住。
   改用显式豁免：移焦前置真，下一轮宏任务解除。判据不依赖浏览器派发细节。 */
var _dpFocusGuard = false;
function _dpClose(){
  ...
  /* v3.7.43：复位被临时抬高的触发输入框（见 _dpOpen）。
     不清掉的话 z-index 会长期滞留在输入框上，可能压住后续浮层。 */
  if(_dpRaisedInput){ try{ _dpRaisedInput.style.zIndex = ""; _dpRaisedInput.style.position = ""; }catch(_e){} _dpRaisedInput = null; }
  _dpInput = null;
  if(back && document.contains(back)){
    /* 防循环：归还焦点会再次触发下方那个 focus 委托 → 面板又打开。
       用一个一次性标志把这次程序化聚焦排除掉。
       v3.7.43：同时开 _dpFocusGuard —— 归还焦点会让**面板内的日格 blur**（若焦点在日格上），
       虽然 blur 委托只认日期输入框、不会命中，但保持"程序化移焦一律豁免"的一致性更安全。 */
    _dpReFocusing = true;
    _dpFocusGuard = true;
    try { back.focus(); } catch(_e){}
    setTimeout(function(){ _dpReFocusing = false; _dpFocusGuard = false; }, 0);
  }
}
...
function _dpOpen(input){
  ...
  /* v3.7.43：遮罩层不再"吃掉"触发输入框的点击 —— 否则日期框**无法用鼠标点回去继续输入**
     （用户诉求"也能输入"，但点第二下就被 overlay 拦住，实测 Playwright
     报 `<div class="dp-overlay"></div> intercepts pointer events`）。
     做法：
       ① overlay 保留（点空白处仍能关闭面板，这是它存在的理由）；
       ② 但给触发输入框临时抬到 overlay 之上（z-index 面板同级），
          这样点是能点、敲是能敲，而点其它任何地方依旧走 overlay 关闭。 */
  overlay.addEventListener("mousedown", function(){ _dpClose(); });
  try{ input.style.position = "relative"; input.style.zIndex = "calc(var(--z-modal, 3000) + 2)"; _dpRaisedInput = input; }catch(_e){}
  ...
  /* v3.7.39：打开后**把焦点移入当前日格** —— 否则焦点仍停在输入框上，
     ...
     v3.7.43：**仅在"键盘打开"时移焦**。用户要是想手输日期，焦点被抢走就没法敲了
     （实测：抢焦后键盘输入全部落到日格上）。
     判定：本轮是否已知是键盘唤起 —— Enter/空格/↓ 打开时把 _dpOpenByKeyboard 置真。 */
  const _kb = _dpOpenByKeyboard;
  _dpOpenByKeyboard = false;
  if(_kb){
    /* v3.7.43：键盘打开时把焦点移入当前日格（roving tabindex 标准配套）。
       ⚠️ 这里**必须同步**执行（不能放 setTimeout），且必须开 _dpFocusGuard 豁免 ——
       否则输入框的 blur 会命中"失焦关面板"委托，把刚建好的面板当场关掉
       （实测：panelOpen:false；详见 _dpFocusGuard 声明处的注释 B）。 */
    _dpFocusGuard = true;
    try {
      const el = panel.querySelector('.dp-day[tabindex="0"]');
      if (el) el.focus();
    } catch (_) {}
    setTimeout(function(){ _dpFocusGuard = false; }, 0);
  }
  /* 鼠标路径的"全选已有值"不在这里做 —— 见下方 focus 委托（那里能覆盖
     "面板已打开时点回来"的场景，而 _dpOpen 对已打开的面板会直接早退）。 */
  /* v3.7.21：面板宽度**跟随触发输入框**（用户 2026-09-23 截图标注：
     "下拉框和日期卡片的宽度应该一致，和右侧的优先级下拉框间距不变"）。
     v3.7.47：下限 200 → **150**。用户 2026-09-25 截图再次强调"选择框和日期卡片宽度应该一致"——
     记录卡表单改 4 列后触发框约 160~170px，若下限仍是 200，面板会比输入框宽出 30~40px，
     正是截图里"日期卡片和选择框不齐"的观感来源。
     日历内部是全流式的（.dp-grid = repeat(7,1fr)、.dp-day = width:100%），150px 下
     表头（‹ 2026年 9月 ›）与 7 列日格仍容纳得下（表头约 129px + 内边距 16px）。
     上限 300 → 320：超宽字段下面板也不至于失真拉长。 */
  const _iw = (input && input.getBoundingClientRect) ? input.getBoundingClientRect().width : 240;
  panel.style.width = Math.min(320, Math.max(150, Math.round(_iw))) + "px";
  _dpLayout(true);
}
...
/* v3.7.43：区分"鼠标点开"与"键盘 Tab 到"：
   鼠标点开 → 焦点留在输入框，用户可以直接接着敲日期；
   键盘聚焦 → 把焦点移进日格，←→↑↓ 立刻可用（roving tabindex 的标准配套）。
   用一个 pointerdown 标记判定：鼠标按下的那一刻置真，focus 事件里读它。
   （Playwright 的 click / 真人的鼠标点击都会先派发 pointerdown；
     Tab 键聚焦不会，因此能干净区分。） */
var _dpPointerDown = false;
document.addEventListener("pointerdown", function(){ _dpPointerDown = true; setTimeout(function(){ _dpPointerDown = false; }, 0); }, true);
document.addEventListener("focus", function(e){
  const t2 = e && e.target;
  if(_dpReFocusing) return;   /* v3.7.39：关闭时归还焦点的那次聚焦，不应重新打开面板 */
  if(t2 && t2.matches && t2.matches('input[data-date-picker="1"]')){
    const byMouse = _dpPointerDown;
    _dpOpenByKeyboard = !byMouse;   /* 不是鼠标按下的 → 当作键盘操作 */
    _dpOpen(t2);
    /* v3.7.43：鼠标点进已有值的日期框时**全选**，否则光标落在点击处、接着敲就成"插入"。
       实测：预填 2026-09-24 时点进去光标 index=7（末尾），键入 "2026-10-01"
       得到 `2026-092026-10-01-24`（见 _probe/diag-caret.txt）。
       放在 focus 里（而非 _dpOpen 内部）是因为面板**已经打开**时 _dpOpen 会早退，
       而"点回来继续改"正是需要全选的第二种场景。
       setTimeout(0)：等浏览器完成点击自带的光标定位后再覆盖选区。 */
    if(byMouse){
      setTimeout(function(){
        try{ if(t2.value && document.activeElement === t2) t2.select(); }catch(_e){}
      }, 0);
    }
  }
}, true);
/* v3.7.43：日期框改为**可手输**（用户："应该是能选择，也能输入的吧"）。
   ── 为什么之前不能输 ──
   ① 模板写了 inputmode="none"（本意"移动端不弹软键盘"），桌面端把键盘输入一并吞掉；
   ② 且没有任何 input 监听把敲进去的字符解析成日期 → 面板也不会跟着跳。
   实测：点击后面板正常弹出，但键入 "2026-10-01" 后 input.value 仍是 ""（整串丢弃）。
   ── 现在 ──
   · 键盘可直接敲 YYYY-MM-DD（也接受 YYYY/MM/DD、YYYY.MM.DD、YYYYMMDD，见 _dpParseLoose）；
   · 输入合法日期 → 面板**实时跳到该月**并把该日高亮（所见即所得，边敲边看）；
   · 输入重开面板时以当前值为准（_dpOpen 已用 _dpParseVal）；
   · 非法的中间状态（如只敲了 "2026-"）不报错、不打断，只是面板暂不跳转；
...
function _dpParseLoose(v){
  ...
  const hh = m[4] === undefined ? null : +m[4];
  const mi = m[5] === undefined ? null : +m[5];
  /* 年份给个合理区间，防止 "0000"/"9999" 这类误敲 */
  if(!(y >= 1900 && y <= 2999)) return null;
  if(!(mo >= 1 && mo <= 12)) return null;
  const dim = new Date(y, mo, 0).getDate();     /* 该月实际天数，自动处理闰年 */
  if(!(d >= 1 && d <= dim)) return null;
  if(hh !== null && !(hh >= 0 && hh <= 23)) return null;
  if(mi !== null && !(mi >= 0 && mi <= 59)) return null;
  return { y: y, m: mo - 1, d: d, h: hh, min: mi };
}
document.addEventListener("input", function(e){
  const el = e && e.target;
  if(!el || !el.matches || !el.matches('input[data-date-picker="1"]')) return;
  const cur = _dpParseLoose(el.value);
  if(!cur) return;                       /* 半成品不处理，等用户敲完 */
  /* 日期合法 → 让面板跟随（重开会把视图月设为该值所在月并高亮该日） */
  if(_dpPanel && _dpInput === el){
    if(cur.y !== _dpViewY || cur.m !== _dpViewM){ _dpViewY = cur.y; _dpViewM = cur.m; _dpRender(); }
  }
}, true);
/* 失焦规范化：只认「完整且合法」的值；敲了一半就离开的，清掉而不是留着误导。
   v3.7.43 补：blur 时**同时关闭面板**。
     仅规范化而不关面板会留下一个尴尬状态 —— 值已经写好了，日历却还开着、
     且 .dp-overlay 仍盖住全屏，用户下一次点击会先被 overlay 吃掉（表现为"点不动"）。
     这正是自动化测试里暴露出来的：第二次想点输入框，点击落在了 overlay 上。
     Esc 键走的是另一条路径（keydown → _dpClose），不受影响。 */
document.addEventListener("blur", function(e){
  const el = e && e.target;
  if(!el || !el.matches || !el.matches('input[data-date-picker="1"]')) return;
  if(_dpFocusGuard) return;             /* v3.7.43：内部程序化移焦 → 放行，不关面板（见下方注释 B） */
  const raw = String(el.value || "").trim();
  if(!raw){
    if(_dpPanel) _dpClose();           /* 空值也收面板，行为一致 */
    return;
  }
  const cur = _dpParseLoose(raw);
  if(!cur){
    el.value = "";                     /* 非法 → 清空，避免把脏值当日期提交 */
    if(_dpPanel) _dpClose();
    return;
  }
  const norm = el.hasAttribute("data-time-picker")
    ? _dpFmt(cur.y, cur.m, cur.d, cur.h === null ? (_dpTimeH === null ? 9 : _dpTimeH) : cur.h,
             cur.min === null ? (_dpTimeM === null ? 0 : _dpTimeM) : cur.min)
    : _dpFmt(cur.y, cur.m, cur.d);
  if(norm !== el.value){
    el.value = norm;
  ...
  meeting: function(){
    return _featureCardHtml({
      ...
      fields:[
        ...
        /* v3.7.43 修复：v3.7.32 把「日期」与「开始时间」拆成两个正交字段（core.js 的会议**记录卡**
           已改成 date + startTime），但**管理卡这里只改了半截** ——
           只在 cols 里加了 {k:"startTime"} 列，fields 里却没补对应的输入框，
           于是 startTime 有列、无输入 → **永远无法录入开始时间**，表格该列恒为空。
           同时 date 还挂着 timePicker:true（"时分藏在日期面板里"的旧设计残留），
           与拆分后的语义重复。这里补齐：拆成 date（纯日期）+ startTime（288 项可编辑时间下拉）。 */
