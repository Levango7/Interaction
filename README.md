...
- **6 个场景 subagent**：办公 / 数据 / 设计 / 学习 / 编程 / 生活，左侧导航一键切换——覆盖知识工作主线 + 日常事务兜底，无娱乐等无关干扰项。（v2.1.0 由 4 场景扩展为 6 场景并改为四组两级菜单；v2.4.0 侧栏四组定案：总览 / 场景 / 应用 / 系统。）
- **顶部「今天要处理」**：所有场景带截止日期的任务自动汇总——今日到期与逾期任务（截止日 ≤ 今天）都计入今日待办，逾期标红 + 一键完成。
- **每场景两个基础模块**：任务看板（待办 / 进行中 / 已完成，按钮移动）+ 场景专属资料库（办公→会议纪要、数据→数据记录、设计→作品记录、学习→学习资料、编程→代码片段、生活→生活记录）。
...
- **侧栏「工具」分区（v2.0 新增）**：萌宠 / 闹钟 / 天气一键直达弹窗，笃行 / 时间追踪弹出浮层——专注工具不再藏在应用页深处。
...
- **数据安全**：导出 / 导入 / 清空（清空二次确认）统一收进设置抽屉「数据管理」；累计 30 条顶部提示备份；顶栏保留搜索 / 命令 / 消息 / 主题 / 下载 / 安装六枚快捷按钮（"更多"按钮 v1.15 已移除，指南入口在侧栏底部，设置 / AI / 文档在侧栏系统组）。
- **响应式布局**：4 断点全分辨率适配——
  - 移动端 `<768px`：底部 Tab 导航，按钮 ≥44px、输入框 ≥16px，适配 iPhone 安全区；（v2.1.0：底栏改为四组按钮 + 底部抽屉，断点统一为 ≤767px。）
...
`1` 办公 · `2` 数据 · `3` 设计 · `4` 学习 · `5` 编程 · `6` 生活（1-6 切场景）· `G` 总览 · `N` 聚焦新建任务 · `Ctrl/Cmd+K` 命令面板。
