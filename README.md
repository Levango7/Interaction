...
- **6 个场景 subagent**：办公 / 数据 / 设计 / 学习 / 编程 / 生活，左侧导航一键切换——覆盖知识工作主线 + 日常事务兜底，无娱乐等无关干扰项。侧栏四组：总览 / 场景 / 应用 / 系统（v2.5.0 菜单精简后定案：总览组 = 主页 / 任务 / 习惯链，系统组 = 回收站 / 设置 / 说明）。
...
- **机制**：暗色模式、命令面板（Ctrl/Cmd+K）、快捷键帮助面板（`?` 唤起）、每日播报、任务标签、Toast 通知。
...
`1` 办公 · `2` 数据 · `3` 设计 · `4` 学习 · `5` 编程 · `6` 生活（1-6 切场景）· `G` 总览 · `N` 聚焦新建任务 · `Ctrl/Cmd+K` 命令面板 · `Ctrl/Cmd+Z` 撤销 · `Esc` 关闭最上层弹窗 · `?` 快捷键帮助面板。
