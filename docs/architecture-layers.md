...
> 本文件是"渐进拆模块"第一里程碑的产物：为 `agent-workbench.html`（~24,000 行单文件）
...
| 段 | 行区间（当前） | 内容 |
|---|---|---|
| `<style>` 样式层 | ~21 – 2349 | 全部 CSS（令牌体系 / 布局 / 组件 / 主题 / 断点） |
| 静态 HTML 骨架 | ~2350 – 3112 | 顶栏 / 侧栏 popover / 抽屉 / 弹窗 / 模板 |
| `<script>` 脚本层 | ~3113 – 24127 | 全部 JS（分层见下） |
