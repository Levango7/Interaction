...
| `render-scene-main` | Render | `ai-retry` `core` `data-idb` `data-links` `data-rw` `render-overview` `render-scene-sub` `render-widgets` `ui-theme` `util-perf` | 21 |
| `render-overview` | Render | `ai-retry` `chain` `core` `data-links` `render-entry` `render-scene-main` `render-scene-sub` `render-widgets` `ui-backup-stats` `ui-drawer` `ui-global-events` `ui-scene-bind` | 30 |
...
| `ui-backup-stats` | UI | `chain` `crypto` `data-idb` `data-links` `data-migrate` `data-rw` | 19 |
| `ui-drawer` | UI | `ai-retry` `core` `crypto` `data-links` `data-migrate` `render-widgets` `ui-daily` `ui-global-events` `ui-guide` `ui-theme` | 32 |
...
- 跨块重复定义：**0** 项
- 循环依赖：**41** 条（data-links → render-scene-main → data-links；data-links → render-scene-main → render-scene-sub → ui-scene-bind → ui-backup-stats → data-links；data-links → render-scene-main → render-scene-sub → ui-scene-bind → ui-backup-stats → data-migrate → data-links；ui-backup-stats → data-migrate → ui-backup-stats；data-links → render-scene-main → render-scene-sub → ui-scene-bind → ui-backup-stats → data-rw → data-links）
- 逆层依赖（低层用高层符号）：**38** 条
