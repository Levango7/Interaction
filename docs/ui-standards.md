...
| 载体 | 是否支持 `var()` | 正确写法 |
|---|---|---|
| CSS 规则 | ✅ | `font-size:var(--fs-sm)` |
| 内联 `style=` | ✅ | `style="font-size:var(--fs-sm)"` |
| **SVG `font-size` 属性** | ✅ **实测支持** | `font-size="var(--fs-sm)"` |
| **Canvas `ctx.font`** | ❌ 不参与 CSS 级联 | 运行时读值：`_pfCssFont("--fs-display-sm","32px")` |

> SVG 属性支持 `var()` 这件事**曾被我误判为不支持** —— 实测（`_probe/svg-var-test.mjs`）
> `font-size="var(--fs-4xs)"` 的 computed 就是 `9px`。**不要靠记忆判断浏览器能力，跑一下。**
> Canvas 确实不支持，但项目已有 `_pfCssColor` 先例，同思路加 `_pfCssFont` 即可，
> 这样 Canvas 字号也进令牌体系，且**改令牌能连带生效**。

**豁免：思维导图三级层次字号**（唯二允许的裸 SVG 字号）

`.mindmap` 的三个层级刻意采用 14 / 12 / 10px 递进：中心节点（r=40）→ 场景节点（r=25）→ 叶子。
**12px 不在阶梯内**（`--fs-2xs`=11 与 `--fs-xs`=13 之间），但**刻意保留**：

- 它承载的是「层级第 2 档」这一**相对关系**，不是绝对刻度
- 改 11px 或 13px 都会破坏与中心/叶子的视觉递进（实测三者渲染宽度差 4~8px，肉眼可辨）
- 实测三种字号在 r=25 圆内**均溢出**（文字 45~54px vs 圆 50px 直径）——
  说明此处字号选择是**有意的视觉密度决策**，不是"塞不下才缩小"

⚑ 守护：`ui-spec-guards.test.js` 断言 `font-size="N"` 裸值**只允许出现在 `ALLOWED` 白名单**，
...
| 令牌 | 用量 | 判定 |
|---|---|---|
| `--fs-xs` / `--fs-sm` | 202 / 185 | 正文主力，健康 |
| `--fs-base` / `--fs-2xs` / `--fs-lg` / `--fs-3xs` / `--fs-xl` / `--fs-md` / `--fs-4xs` | 55 / 35 / 30 / 20 / 18 / 11 / 9 | 梯度递减，健康 |
| `--fs-2xl` | 4 | 偏少但每处独立（卡片大数值 + 抽屉关闭键） |
| `--fs-display`(36) / `--fs-3xl`(22) / `--fs-display-lg`(48) / `--fs-display-sm`(32) | 2 / 2 / 1 / 1 | **展示级，低频是本质属性** |

> 🔴 **不要把"低频"误判为"虚胖"**：展示级档位（hero 数字、闹钟时间、天气温度）
> 天然是「一页一个」——全站只有一个闹钟，`--fs-display-lg` 当然只被引用 1 次。
> **减档的收益是"阶梯数字好看"，成本是真实视觉回归**（相邻档合并必然在一处产生
> 8~30% 的字号变化）。收益虚、成本实，**不做**。
>
> 阶梯小端 1~2px 密集（9/10/11/13/14/15/16）也是**刻意的精细排版**，
> 每档对应不同场景（顶栏 10 / 注释 11 / 小标题 13 / 正文 14 / 长文 15 / 区块标题 16），
> **不是冗余**。
