...
### 产品定位定调：云方向（server 保留并排期对接）+ README 口径修正

**📍 定位决策**（用户拍板）
- 产品走「**本地优先 + 可选云端**」：核心功能保持纯本地 PWA 离线可用；`server/` 账号系统保留为实验性可选能力，后续排期做前端对接（云同步 / 推送 / 集成代理）。
- README 冲突表述修正：~~「纯本地；不依赖任何后端服务器」~~ → 「本地优先 + 实验性可选云端服务」；~~「无账号体系」~~ → 「账号体系（实验性，云方向），尚未与前端对接，不启用即纯本地」。
- 顺带修正 README 版本引用滞后（v3.4.4/v3.4.2 → v3.4.8）。
- 根目录测试残留（~5.3MB gitignored 文件）经用户确认**暂不清理**，仅记录在案。
...
**🌐 i18n JS 拼接中文 t() 替换（约 300+ 处）**
- **区域 1 · 看板/任务卡片**（Line 5000-9000）：场景定义、sysprompt、err 字符串、回收站、会话管理、execTool 消息、agentExec 消息、agentContextPrompt、备份相关等。
- **区域 2 · 弹窗/对话框**（Line 11000-15000）：图片/视频生成表单、网页搜索、代码运行器、正则测试器、md2code、天气渲染、闹钟、萌宠、表单验证、骨架屏、空状态等。
- **区域 3 · 设置面板**（Line 15000-20000）：任务表单、待办筛选、功能渲染、错题复习、项目/考勤/计划/缴费统计、复制/删除确认、批量操作、危险操作确认、AI 记忆/工具调用、聊天渲染、附件、AI Hub、每日播报等。
- **区域 4 · 侧边栏/导航**（Line 20000-23720）：集成配置、链分享/导入、插件管理、配置保存 toast、语言切换、通知提醒、联动规则、主题切换、推送订阅、同步状态等（部分完成）。
- 新增 100+ 个 MESSAGES key（zh + en），包括 `scene.*`、`field.*`、`common.*`、`tool.*`、`ai.*`、`int.*`、`msg.*`、`cmd.*`、`status.*` 等。
- 修复子代理引入的 `}else{` → `}catch(e){` 语法错误（Line 22141）。
...
**🌲 秘境森林——从浅灰绿改为深墨绿底色风格（用户反馈"还是很寡淡"）**
- 底布 `#e8f3e0`（浅灰绿）→ `#1b2d20`（深墨绿）；面板 `#f9fcf6`→`#243528`；文字 `#0f1f15`→`#e8f3e0`（浅绿白）。
- 主色 `#1e8448`→`#4ade80`（鲜翠绿，深底上高亮）；副色 `#d68910`→`#f0a030`（琥珀金）。
- 侧边栏不透明度 50%→78%（深底需更不透明）；聊天面板 58%→75%。
- canvas-glow 适配深色底：光斑透明度 0.40–0.58→0.25–0.40（深底上不需太高透明度即显眼）。
- sidebar-glow / topbar / nav-item / chat-panel 所有 rgba 引用同步更新。

**🏷️ 三个主题改名**
- 幽蓝海洋 → **微蓝浅海**（下拉/PRESET/i18n 三处 + 补 data-i18n）。
- 极光 → **幻夜极光**（下拉/PRESET/i18n 三处 + 补 data-i18n）。
- 初雪 → **见静初雪**（下拉/PRESET/i18n 三处）。
- aurora/forest/ocean 下拉选项补 data-i18n 属性 + 中英文翻译（Mystic Forest / Pale Blue Sea / Aurora Night）。

**🌐 i18n**
- 英文 i18n key 与中文仅差 1 个（`chainPage.subExtra`），将在 v3.4.5 i18n 硬编码补全中一并处理。
- 902 处硬编码中文文本（无 data-i18n）将在 v3.4.5 批量补全。

**✅ 验证**：lint 0 problems；硬编码颜色 0；build:check 门禁通过。

## [v3.4.3] - 2026-09-04

### 秘境森林加鲜艳 / 幽蓝海洋加深 / 护眼改名温和护眼

**🎨 秘境森林——颜色加鲜艳（用户反馈"太寡淡"）**
- 主色 `#2d6a4f`（偏灰绿，饱和度 39%）→ `#1e8448`（翠绿，饱和度 63%）。
- 副色 `#c47a3a`→`#d68910`（琥珀金，更鲜亮）；底色 `#eef3ea`→`#e8f3e0`（绿意更浓）。
- canvas-glow 翠绿层提饱和：rgba(88,178,98)→rgba(52,196,84)，琥珀层 rgba(226,138,66)→rgba(232,150,40)。
- sidebar-glow / topbar / nav-item / chat-panel 所有 rgba 引用同步更新。

**🌊 幽蓝海洋——颜色加深**
- 主色 `#0f6b7e`→`#0a5263`（更深青蓝）；副色 `#d97862`→`#c9684f`（深珊瑚）。
- 底色 `#e8f1f4`→`#dce8ee`（加深）；canvas-glow 主光 rgba(16,150,176)→rgba(12,130,155)。
- 所有 rgba 引用同步更新。

**🏷️ 护眼模式→温和护眼**
- 下拉选项 / 中文 i18n / PRESET_THEMES 三处改名"护眼模式"→"温和护眼"。
- 英文 "Sepia" 保留不动。

**✅ 验证**：lint 0 problems；硬编码颜色 0；build:check 门禁通过；quickwins 22/22（含 forest/ocean 回归）。

## [v3.4.2] - 2026-09-04

### 秘境森林 / 幽蓝海洋 光效加浓——向极光量级看齐

**🎨 canvas-glow 透明度大幅提升（用户反馈"太寡淡"）**
- 原森林/海洋光斑透明度仅 0.16–0.30（与护眼/初雪同档），极光是 0.52–0.78——配错了档位。
- **秘境森林**（5 层，0.38–0.55）：树冠阳光绿 rgba(88,178,98,.55) + **新增丁达尔晨光** rgba(240,180,80,.42)（顶部金色光柱）+ 暖木橙 .48 + 底部苔藓深绿 .38 + 侧苔藓浅绿 .40。
- **幽蓝海洋**（5 层，0.42–0.55）：深海青 rgba(16,150,176,.55) + **新增晨曦金** rgba(245,185,110,.45)（左上海面日出）+ 珊瑚粉 .45 + 底部海面青 .42 + 月光白 .52。
- sidebar-glow 同步加浓（.12/.06/.04 → .20/.10/.07），侧栏氛围更浓。
- 文字对比度仍 ≥ 4.5:1（光斑在视口边缘，panel 白底覆盖主体内容区）。
...
**🆕 New**
- **主页「系统概况」卡**（放习惯链可视化左侧，AI 助手 Hub 移至最下跨两列）：新手可读的系统/业务全貌——场景 / 任务 / 已完成 / 资料 / 笔记 / 习惯链 / 插件 七个数字项（icon + 数字 + 标签，点击直达对应页面）+ AI 助手启用状态行（未启用时给「去设置」引导，跳设置→AI）+ 新手提示行
- **19 个占位工具全部上线最小可用版**（`TOOL_APPS` 注册表 + `openToolStub` 分发：命中渲染真实工具页，未注册仍落占位页；数据键 `wb_agent_tool_*` 纯前端离线）：
  - 文档簇 6：Markdown 编辑器（分栏预览/导出 .md .html）、Word 富文本（execCommand 工具栏/导出 html doc）、表格编辑器（行列增删/CSV 导出）、幻灯片（缩略图+编辑+全屏放映/JSON 导出）、PDF 阅读（本地导入 iframe 预览）、截图 OCR（AI 多模态识别，未配置给引导）
  - 设计簇 4 + 检索 1：CAD 画布（线条/矩形/圆/多边形/撤销/导出 SVG PNG）、图片滤镜（灰度反色怀旧模糊/还原/PNG 导出）、AI 文生图、AI 视频生成（配置引导）、在线检索（跨任务/资料/场景本地全文检索）
  - 编程簇 3：代码运行器（沙箱执行+console 捕获+历史）、正则测试器（捕获组列表+高亮+常用模式）、Markdown 转代码（HTML/JSON 大纲/纯文本）
  - 生活簇 5（公共 CRUD 骨架 `_recordToolHtml`/`_bindRecordTool`）：运动记录（周月统计）、缴费提醒（逾期标红/即将到期黄标/待缴总额）、采购清单（已购勾选/预算小计）、外卖记录（月消费/常点 TOP3）、出行记录（方式分布/月费用）
- 新增 `tests/tools.test.js` 16 用例（注册表完整性 / openToolStub 分发 / 代表工具冒烟 / 改名断言 / 概况卡顺序）

**🔧 Changed**
- **仪表盘入口改名**：侧栏子项「总览图」→「**编辑**」（数据场景「图表」同步改），点击直达统计页编辑态（`_dashEditMode=true`）；标题栏展示态「统计」→「**仪表盘**」、编辑态「**编辑仪表盘**」，副标题随态切换
- **图表商店废弃**（用户拍板并入仪表盘）：侧栏与数据场景不再指向 `openChartStore`（函数保留兜底防插件引用报错）；7 种图表能力已由仪表盘组件体系覆盖

**📋 Technical**
- `openToolStub(toolId)` 重构为 TOOL_APPS 查表分发；工具页结构复用 page-head 卡 + `tool-app-card` 正文；新增工具页 CSS（表格/表单网格/摘要条）
- canvas 取色走 `_pfCssColor()` 令牌化（lint-colors PASS 0 违规）；主页概况数字项事件绑定进 renderOverview
- 版本号 v2.2.1 → **v2.3.0**（六处同步，build tag 20260822c → 20260823a）

## [v2.2.1] - 2026-08-22

### Patch：主页核心业务重构（R2）+ 仪表盘高度自适应与组件多元化

**🔧 Changed**
- **主页重构 R2**（针对 v2.2.0 布局仍乱的反馈）：
  - 「今日待办」Top5 列表进主页核心区（按优先级排序，逾期标红，点击条目直达对应场景；>5 项显示「还有 N 项」入口）
  - 各场景进度条并入今日待办卡（核心业务聚合）；数据速览卡瘦身（趋势 + 热力图）
  - 插件卡从主页移除（低频内容，插件有侧栏入口）；「高级报表」按钮上移页头
  - 布局：KPI 条 → 全局搜索（一行）→ 今日待办｜数据速览双列 → AI 助手 Hub｜习惯链双列
- **仪表盘高度自适应**：grid 行高 120px 固定 → `minmax(150px, auto)`，widget-body `overflow:auto` → `visible`——组件内容完整可见，不再出现内部滚动条
- **仪表盘组件多元化**（不止图表）：新增「今日待办」（列表）、「场景概况」（进度条）、「最近完成」（列表+时间）、「快捷操作」（新建/搜索/报表/导出按钮组）四个非图表组件；默认布局 8 组件（含 3 个非图表）；≤767px 单列降级

**📋 Technical**
- 新增 `_renderTodayTopHtml()`（主页与 today 组件共用）；`_bindDashToolbar` 补快捷操作绑定
- 版本号 v2.2.0 → **v2.2.1**（六处同步，build tag 20260822b → 20260822c）
- 实测：今日待办 3 条含逾期标记、点击跳办公场景、仪表盘 0 溢出、可添加 7 组件；vitest 606/606
